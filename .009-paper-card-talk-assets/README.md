...
- 模型：OpenAI gpt-image-2，通过 ncds 主机的 `~/.codex/skills/gpt-image/scripts/gpt_image_gen.py`
- 路径：`ssh ncds@100.74.164.89 → source ~/.zshrc → gpt_image_gen.py --size 1536x1024 --prompt ...`
- 落地：远端生成 PNG → scp 回 → Pillow 转 1536×1024 WebP（quality 85）→ `pictures/NN-<id>.webp`
...
# 6. 跑文生图（需 ncds 可达 + codeproxy.dev 网关健康）
