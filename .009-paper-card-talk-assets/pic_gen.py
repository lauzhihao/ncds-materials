#!/usr/bin/env python3
"""按 SCENES 顺序在 ncds 上调 gpt-image-2 生成插图，回传转 WebP。
...
依赖：本机 Pillow（pip install Pillow），ncds 有 GPT_IMAGE2_BASE_URL 和
GPT_IMAGE2_API_KEY（在 ~/.zshrc）。
"""
import json
import os
import shutil
...
def generate_one(scene: dict, force: bool) -> str:
    ...
    remote_out_dir = f"/tmp/gpt-image/009-{nn}-{sid}"
    remote_cmd = (
        "source ~/.zshrc 2>/dev/null; "
        f"rm -rf {remote_out_dir} && mkdir -p {remote_out_dir} && "
        "python3 ~/.codex/skills/gpt-image/scripts/gpt_image_gen.py "
        f"--out-dir {remote_out_dir} --size {SIZE} --quality {QUALITY} --overwrite "
    ...
    t0 = time.time()
    res = subprocess.run(
        ["ssh", "-o", "ServerAliveInterval=60", NCDS_HOST, remote_cmd],
    ...
    # scp the PNG back
    remote_png = f"{remote_out_dir}/image_01.png"
    local_png = Path("/tmp") / f"009-gen-{nn}-{sid}.png"
    scp = subprocess.run(
        ["scp", "-O", "-q", f"{NCDS_HOST}:{remote_png}", str(local_png)],
        capture_output=True, text=True, timeout=120,
    )
    if scp.returncode != 0:
        print(f"  ! [{nn}/{sid}] scp failed: {scp.stderr}", file=sys.stderr)
        return "failed"
