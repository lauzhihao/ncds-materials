...
用法（在仓库根目录跑）：
    python3 .019-draft-assets/edit-server.py                # 默认开热重载
    python3 .019-draft-assets/edit-server.py --no-watch     # 关掉
然后访问 http://127.0.0.1:8765/019-draft.html，按 E 进入编辑模式。
"""
import argparse
import json
import os
...
def watcher_thread():
    ...
    def filt(change_type, path):
        ...
        if any(p in WATCH_EXCLUDE_DIRS for p in parts):
            return False
        if any(p.startswith('.') and p not in {'.019-draft-assets', '.012-not-fooled-assets',
