...
// slug 从所在资源目录名推出（.019-draft-assets → 019-draft），复制开新素材时本文件无需改
...
async function main() {
  ...
  try {
    ...
    log('loading page');
    // 用 'load' 而非 'networkidle0'：dev-reload.js 的 EventSource('/__reload_events')
    // 是常驻 SSE 连接（且会自动重连），networkidle0 永远等不到 0 连接 → 30s 超时。
    // 播放器就绪由下方 waitForFunction(__player.beats) + sleep(1200) 保证，无需 networkidle0。
    await page.goto(URL_LOCAL, { waitUntil: 'load', timeout: 30000 });
