...
新项目命名 `<NNN>-<theme>`。从上一套 `<PREV>-<old>` 复制后，下面这些都要改一遍——大多是 slug 字符串替换，少漏一处线上就会 404。
...
> 资源目录里的文件全部按职责命名（`manifest.json` / `episode.json` / `bootstrap.js` / `player.js` / `styles.css` / `render.mjs` ...），不带 slug 编号——目录名本身已经承载了素材编号，文件名再重复一遍是冗余。新加素材文件时也遵循这条。
...
- **资源目录里的 `manifest.json`**：约 7 处（`id` / `entry` / `style` / `script` / `assets` / `scripts[]`）——文件名固定不变，只换里面的字符串
- **`render.mjs`**：无需改——`SLUG = path.basename(HERE).replace(/^\./, '').replace(/-assets$/, '')` 从所在目录名自动推出 slug，所有常量（`OUTPUT_MP4` / `TMP_*` / `URL_LOCAL` / 静音片段 / concat 列表）都派生自这个值
...
- `episode.json`：`meta.title` / `titleOptions` / `disclaimer` / `beats[]` / `scenes{}` 按新主题重写
...
**目标：每个 scene 控制在 1-3 条 beat。** 一张图停 4 句话以上，Ken Burns 没法在一张图里做出 4 句话的差异化镜头，观众会觉得"画面卡住"——历史上踩过一次 ~4 beat/scene 的坑，拍出来明显比 ~2 beat/scene 那套慢半拍。
...
**拆细的代价**：每多一个 scene 多生一张图（gpt-image-2 几毛一张），多写一段 prompt；overlay 数量不变，但 `at.match` 关键词所属 sceneId 可能要重挂。
